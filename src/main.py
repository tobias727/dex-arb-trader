...
def main():
    ...
    while True:
        ...
        uniswap_bid_notional, uniswap_ask_notional = uniswap.estimate_swap_price() # returns [price, gas]
        logger.info("Uniswap: %s, %s", f"{uniswap_bid_notional[0]:_}", f"{uniswap_ask_notional[0]:_}")
        binance_bid_notional, binance_ask_notional = binance.get_price()
        logger.info("Binance: %s, %s", f"{binance_bid_notional:_}", f"{binance_ask_notional:_}")
        side, edge = detector.detect(binance_bid_notional, binance_ask_notional, uniswap_bid_notional[0], uniswap_ask_notional[0])
        if side:
